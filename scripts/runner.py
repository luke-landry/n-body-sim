...
# Helper to run simulations and the visualizer
# Simulations are run in a subprocess and handled through QProcess
# Visualizer data loading is run in a separate thread via QThread
class Runner(QObject):
    ...
    def __init__(
        ...
        self.show_visualizer = True  # Default to showing visualizer
    ...
    def run_simulator_and_view(self):
        self.show_visualizer = True
        if self.loading_dialog:
            self.loading_dialog.show()
        self.start_sim_process()

    def run_simulator_only(self):
        self.show_visualizer = False
        if self.loading_dialog:
            self.loading_dialog.show()
        self.start_sim_process()

    def run_visualizer_only(self):
        self.show_visualizer = True
        if self.loading_dialog:
    ...
    def start(self):
        # Legacy method for backward compatibility, defaults to simulator and view
        self.run_simulator_and_view()

    def start_sim_process(self):
        if not self.bin_path.is_file():
            self.fail(f"Binary not found: {self.bin_path}")
            return
    ...
    def on_sim_process_finished(self, exit_code, exit_status):
        output = ""
        if self.sim_process:
        ...
        if exit_status == QProcess.ExitStatus.NormalExit and exit_code == 0:
            if self.show_visualizer:
                ...
                self.delete_loading_dialog()
                self.finished.emit()
                self.deleteLater()
        else:
            self.fail(f"Physics Engine Error:\n{output}")


    def on_sim_process_error(self, error):
        output = ""
        if self.sim_process:
            output = self.sim_process.readAllStandardOutput().data().decode(errors="ignore")  # type: ignore
            self.sim_process.deleteLater()
            self.sim_process = None
        self.fail(output or str(error))
    ...
    def start_visualizer_load(self):
        ...
        self._loading_worker.error.connect(self.fail)
    ...
    def on_visualizer_loaded(self, data, config):
        ...
        self.loading_dialog.hide() # type: ignore
    ...
    def delete_loading_dialog(self):
        if self.loading_dialog:
