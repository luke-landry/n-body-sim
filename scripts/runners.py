...
from schema import SimulationData, SimulationParameters, VisualizerConfig
...
class SimulationRunner(Runner):
    ...
    def build_sim_args(self) -> list[str]:
        return [
            ...
            "-g",
            str(self.sim_parameters.g_constant),
            "-t",
            str(self.sim_parameters.time_step),
            "-n",
            str(self.sim_parameters.num_steps),
            "--softening-factor",
            str(self.sim_parameters.softening_factor),
            "--theta",
            str(self.sim_parameters.theta),
            "--gravity",
            self.sim_parameters.gravity,
            "--integrator",
            self.sim_parameters.integrator,
        ]
    ...
    def start_sim_process(self):
        ...
        print(
            f"Started simulation process with PID {self.sim_process.processId()}:\n\targuments: {self.build_sim_args()}"
        )
...
class VisualizerRunner(Runner):
    ...
    def start_visualizer_load(self):
        self._loading_dialog = self.create_loading_dialog(
            title="Loading Visualizer", label="Loading simulation data..."
        )
        ...
        print(
            f"Started visualizer loading thread:\n\tdata: {self.output_path}\n\tconfig: {self.config_path}"
        )
